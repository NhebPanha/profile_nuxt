...
<template>
  <header class="fixed w-full z-20 top-0 duration-200 px-4 border-b border-solid" :class="{
    ...
    <nav class="flex flex-row items-center justify-between max-w-[1400px] mx-auto">
      <div class="flex gap-4 items-center cursor-pointer text-primary hover:text-blue-300" @click="goTop">
        ...
        <h1 class="font-medium text-xl">
          <b class="font-bold poppins mr-1">Panha</b>
          <span class="font-bold poppins ml-1 hidden md:inline-block"> NHEB </span>
        </h1>
      </div>
      <div class="flex gap-4 items-center ml-auto relative" v-motion-fade-visible-once>
        ...
        <button aria-label="expand mobile menu button" @click="isOpen = !isOpen"
          class="md:hidden block p-2 absolute top-0 right-0" :class="{ open: isOpen, 'focus:outline-none': true }">
      ...
      <nav class="items-center flex-row hidden md:flex">
        <div class="flex pr-3">
          <div class="sm:flex items-center gap-4 text-center hidden">
            ...
            <a v-for="link in NAV_LINKS" :key="link.href" class="hover:text-primary hover:underline cursor-pointer"
              :href="link.href" @click.prevent="handleNav(link.href)">
              {{ link.label }}
            </a>
          </div>
        </div>
    ...
    <nav v-show="isOpen" @click="isOpen = false" class="md:hidden p-4 mt-2 flex flex-col gap-2 text-center font-bold"
      ...
      <a v-for="link in NAV_LINKS" :key="link.href"
        class="border rounded p-2 border-primary animate-link cursor-pointer" :href="link.href"
        @click.prevent="handleNav(link.href)">
        {{ link.label }}
      </a>

      <a href="https://linkedin.com/" target="_blank" aria-label="Contact page link"
